...
import hmac
import sys
from typing import Optional, Dict, Any, List, DefaultDict, Deque, Tuple
...
async def ws_fills(msg):
    async with websockets.connect('wss://ftx.com/ws/') as websocket:
        ...
        while websocket.open:
            ...
            if counter == 1:
                # msg2 = {'op': 'subscribe',
                #         'channel': 'ticker', 'market': spot_market}
                msg2 = {'op': 'subscribe', 'channel': 'fills'}
                ...
                if channel == 'ticker':
                    ...
                    print('Order filled ', market,
                          side, price, size, liquidity)
                    if liquidity == 'maker':
                        if side == 'buy':
                            print('Setting sell order for ', market)
                            ftx.place_order(market=market, side='sell', price=(price + (spread * price)), size=size, type='limit',
                                            reduce_only=False, ioc=False, post_only=True)
                        else:
                            print('Setting buy order for ', market)
                            ftx.place_order(market=market, side='buy', price=(price - (spread * price)), size=size, type='limit',
                                            reduce_only=False, ioc=False, post_only=True)
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(ws_fills(json.dumps(msg)))
    loop.run_forever()
